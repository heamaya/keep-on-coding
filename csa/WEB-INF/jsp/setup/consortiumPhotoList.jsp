	...
	<d:table name="${photos}" class="list" uid="row" pagesize="3" sort="list" requestURI="" export="false" excludedParams="mappedRequest descriptionPortuguese descriptionSpanish descriptionEnglish actionMethod">
		...
		<d:column title="Nombre" sortable="true" >
			...
			<sj:a cssClass="listButton" href="%{url}" targets="currentFormDiv" onCompleteTopics="openCurrentDialog"  >
    			<s:property value="%{#attr.row.photoFileName}" />
    		</sj:a> 
 		</d:column>
		...
		<d:column title="Descripción Portugués" sortable="true" >
 		...
 		<d:column>
 			...
 			<s:url id="url" action="ConsortiumPhotoDownloadFileAction_getFile" namespace="/setup" escapeAmp="false">
				<s:param name="fileName" value="%{#attr.row.photoFileName}" />
				<s:param name="contentType" value="%{#attr.row.photoContentType}" />					
				<s:param name="consortiumId" value="%{#attr.row.consortium.id}" /> 
			</s:url>
