		...
		<tr>
			<td>
				...
				<sj:a cssClass="listButton" href="%{#url}" targets="currentFormDiv" button="true" onCompleteTopics="openCurrentDialog" requestType="GET">
					...
					<s:div cssClass="button">
						<img src='<s:property value="%{#iconUrl}" />' title='Agregar Nuevo Libro de Mes' />
					</s:div>					
				</sj:a>
			</td>
			...
			<td>
    			...
    			<sj:a cssClass="listButton" href="%{url}" targets="currentFormDiv" button="true" onCompleteTopics="openCurrentDialog" requestType="GET">
					...
					<s:div cssClass="button">
						<img src='<s:property value="%{#iconUrl}" />' title='Filtrar Libro de Meses' />
					</s:div>					
    			</sj:a>
			</td>
			...
			<td>
    			...
    			<sj:a cssClass="listButton" href="%{url}" targets="result" button="true" requestType="GET" onCompleteTopics="completeRefresh">
					...
					<s:div cssClass="button">
						<img src='<s:property value="%{#iconUrl}" />' title='Refrescar Listado de Libro de Meses' />
					</s:div>					
    			</sj:a>
			</td>
			<td>
	...
	<sj:dialog id="currentDialog" width="600" modal="true" draggable="true" position="center" resizable="true" title="Entrada Libro de Mes" autoOpen="false" onBeforeTopics="beforeOpenCurrentDialog">
		<s:div id="currentFormDiv">
		</s:div>
	</sj:dialog>

	<s:div id="result">
		<jsp:include page="journalList.jsp" />
	</s:div>